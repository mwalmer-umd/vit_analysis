"""
###########################################################################
Gather and plot downstream analysis results for supplemental plots
...
os.makedirs('all_results/plots_all', exist_ok=True)
def plot_knn(topk = 'top1'):
    ...
    pertype_blockwise_comparison_plot('all_results/plots_all', data, names, f'KNN_{topk}_Acc')
...
def plot_spair(metric = 'pck@0.1'):
    ...
    for mod_id in sorted(os.listdir(root_dir)):
        if mod_id!='plots' and mod_id!='davis_dense' and mod_id!='plots_all':
            ...
            data.append(np.array(temp_knn))
    pertype_blockwise_comparison_plot('all_results/plots_all', data, names, f'SPair_{metric}')
...
def plot_retreival(dataset='roxford5k', metric='mapM'):
    ...
    pertype_blockwise_comparison_plot('all_results/plots_all', data, names, f'Retrieval_{dataset}_{metric}')
...
def plot_davis(metric='mapM'):
    ...
    pertype_blockwise_comparison_plot('all_results/plots_all', data, names, f'DAVIS_{metric}')
    best_block_table(data, names, metric, higher_better=True)
