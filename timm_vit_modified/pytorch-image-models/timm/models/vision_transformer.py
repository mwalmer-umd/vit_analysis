...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = self.attn_residual(x) + self.attn_normal(self.drop_path1(self.ls1(self.attn(self.norm1(x)))))
        x = self.mlp_residual(x) + self.mlp_normal(self.drop_path2(self.ls2(self.mlp(self.norm2(x)))))
        return x
