...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn_residual = nn.Identity()
        self.attn_normal = nn.Identity()
        self.mlp_residual = nn.Identity()
        self.mlp_normal = nn.Identity()

    def forward(self, x, shared_rel_pos_bias: Optional[torch.Tensor] = None):
        if self.gamma_1 is None:
            x = self.attn_residual(x) + self.attn_normal(self.drop_path(self.attn(self.norm1(x), shared_rel_pos_bias=shared_rel_pos_bias)))
            x = self.mlp_residual(x) + self.mlp_normal(self.drop_path(self.mlp(self.norm2(x))))
        else:
            x = self.attn_residual(x) + self.attn_normal(self.drop_path(self.gamma_1 * self.attn(self.norm1(x), shared_rel_pos_bias=shared_rel_pos_bias)))
            x = self.mlp_residual(x) + self.mlp_normal(self.drop_path(self.gamma_2 * self.mlp(self.norm2(x))))
        return x
